 ...
 *
 * 프로젝트 하나(정확히는 client-expert 매칭 하나)당 가상번호 하나를 발급하고,
 * 프로젝트가 끝나도 기록은 남겨두되(정산/분쟁 이력 추적 목적 - 감사 로그와 같은 원리),
 * "지금 이 번호가 아직 살아있는지"는 조회 시점에 프로젝트 상태를 같이 보고 판단한다
...
export class SafeNumberMapping {
  ...
  // 프로젝트 하나당 매핑 하나만 존재해야 한다 (재발급이 아니라 재사용).
